	...
	try {
		...
		if (deliveryMode === "pickup") {
			if (locationId == null) {
				return res
					.status(400)
					.json({ error: "Отсутствует locationId для pickup" });
			}
			...
			const deliveryPointResult = await pool.query(deliveryPointQuery, [
				locationId,
			]);
			...
			if (!lat || !lng) {
				return res
					.status(400)
					.json({ error: "Отсутствуют координаты для курьерской доставки" });
			}
			...
			if (!zone) {
				return res
					.status(404)
					.json({ error: "Нет зон доставки для указанных координат" });
			}
...
const createDeliveryZone = async (req, res) => {
	try {
		...
		res.status(500).json({ error: "Ошибка при создании зоны доставки" });
	}
};
...
const findPointsInRadius = async (req, res) => {
	try {
		...
		res.status(500).json({ error: "Ошибка при поиске точек доставки" });
	}
};
...
const findNearestPoint = async (req, res) => {
	try {
		...
		res.status(500).json({ error: "Ошибка при поиске бижайшей точки досавк" });
	}
};
...
const getDeliveryZoneByCoordinates = async (req, res) => {
	try {
		...
		if (!zone) {
			return res
				.status(404)
				.json({ error: "Зона доставки не найдена для указанных координат" });
		}

		res.json(zone);
	} catch (error) {
		console.error(
			"Ошибка при определении зоны доставки по координатам:",
			error
		);
...
const mapTrackingMoreStatus = (tmStatus) => {
...
const trackParcel = async (req, res) => {
	try {
		...
		console.log("Courier Code:", "china-ems");
		console.log("API Key:", TRACKINGMORE_API_KEY ? "Present" : "Missing");

		const getResponse = await axios({
			method: "get",
			url: `https://api.trackingmore.com/v4/trackings/${trackingNumber}/china-ems`,
			headers: {
		...
		// Обновляем статус заказа на основе данных от TrackingMore
		const trackingStatus = getResponse.data?.data?.delivery_status;
		if (trackingStatus) {
			const mappedStatus = mapTrackingMoreStatus(trackingStatus);
			await pool.query(
				"UPDATE orders SET status = $1 WHERE tracking_number = $2",
				[mappedStatus, trackingNumber]
			);
		}
		...
		if (error.response?.status === 404) {
			try {
				...
				const trackingResponse = await axios({
					...
					data: {
						...
						title: `Order Tracking #${trackingNumber}`,
						destination_country_iso2: "RU",
				...
				console.log("Ответ от TrackingMore:", trackingResponse.data);
				res.json(trackingResponse.data);
			} catch (error) {
				console.error(
					"Полная ошибка при создании отслеживания:",
					error.response?.data || error
				);
				res.status(500).json({ error: "Ошибка при создании отслеживания" });
			}
		} else {
			console.error(
			...
			res.status(500).json({ error: "Ошибка при отслеживании посылки" });
		}
	}
};
